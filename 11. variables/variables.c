...
lval* lenv_get(lenv* e, lval* k){
	...
	return lval_err("unbound symbol '%s'", k->sym); // no symbols found to match, hence error is returned;	
}
...
lval* lval_eval_sexpr(lenv* e,lval* v){
	...
	if (f->type != LVAL_FUN){
		lval_del(f);
		lval_del(v);
		return lval_err("first expression isnt a function!");
	}
...
void lenv_add_builtin(lenv* e, char* name, lbuiltin func){
	...
	lval* v = lval_fun(func); // making a function reference/clone out of the func
	lenv_put(e, k, v);
	lval_del(k);
	lval_del(v);
}
